...
void makeDistr() {
  ...
  const string outputFile = "make_dat.root";
  ...
  Int_t npf, reco_n, nvtx, gpn, itype, pass[10], mishitspf[10], reco_mishits[10];
  ...
  out_tree->Branch("hlt_n" , &npf, "hlt_n/I");
  ...
  for (int nfiles = 0; nfiles < proc; nfiles++) {
	...
	hlt_phi[i] = phipf[i];
	hlt_sie[i] = sieiepf[i];
	hlt_ecc[i] = ecalpf[i] / etpf[i];
	hlt_dph[i] = dphipf[i];
	hlt_det[i] = detapf[i];
	hlt_des[i] = detaseedpf[i];
	hlt_hoe[i] = hoepf[i] / epf[i];
	hlt_hcc[i] = hcalpf[i] / etpf[i];
	hlt_tki[i] = tkisopf[i] / etpf[i];
	hlt_eop[i] = eoppf[i];
	hlt_chi[i] = chi2pf[i];
        hlt_mih[i] = (Float_t) mishitspf[i];

        if (fabs(hlt_eta[i]) < 1.4791) {

          if(barWPLoose(hlt_sie[i], hlt_hoe[i], hlt_ecc[i], hlt_hcc[i], hlt_eop[i], hlt_chi[i], hlt_mih[i], hlt_det[i], hlt_dph[i], hlt_tki[i]))
            pass[i] = 1;

          // Recompute the non-corr iso
          hlt_ecu[i] = (ecalpf[i] + (0.16544 * rho)) / etpf[i];
          hlt_hcu[i] = (hcalpf[i] + (0.05956 * rho)) / etpf[i];

        }

        else if (fabs(hlt_eta[i]) >= 1.4791) {

          if(endWPLoose(hlt_sie[i], hlt_hoe[i], hlt_ecc[i], hlt_hcc[i], hlt_eop[i], hlt_chi[i], hlt_mih[i], hlt_det[i], hlt_dph[i], hlt_tki[i]))
            pass[i] = 1;

          // Recompute the non-corr iso
          hlt_ecu[i] = (ecalpf[i] + (0.13212 * rho)) / etpf[i];
          hlt_hcu[i] = (hcalpf[i] + (0.13052 * rho)) / etpf[i];

        }

        hlt_eca[i] = hlt_ecc[i] * etpf[i];
        hlt_hca[i] = hlt_hcc[i] * etpf[i];

      }

      if (hasReco) {

        for (int j = 0; j < reco_n; j++) {
