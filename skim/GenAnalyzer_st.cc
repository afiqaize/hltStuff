...
// user include files
#include "GeneratorInterface/GenFilters/interface/GenAnalyzer_st.h"
...
//
GenAnalyzer_st::GenAnalyzer_st(const edm::ParameterSet& iConfig):
...
GenAnalyzer_st::~GenAnalyzer_st() {

	 // do anything here that needs to be done at desctruction time
	 // (e.g. close files, deallocate resources etc.)
}

void GenAnalyzer_st::resetTree() {
...
const reco::Candidate* GenAnalyzer_st::getGeneratorFinalStateFor(const reco::Candidate *p, bool isSherpa) {
...
bool GenAnalyzer_st::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	for (size_t i = 0; i < genParticles->size(); ++i ) {
		...
		if (p.status() != 3 && (p.status()<20 || p.status()>30) ) continue;
                ...
                // Dump the light jet properties
		if(abs(p.pdgId()) <=  4 && (p.status()==3 || p.status()==23 /*|| p.status()==24*/)){
		   
                   const reco::Candidate *lightJet = getGeneratorFinalStateFor(&p); // Find the light jet

                   quark_pts.push_back(lightJet->pt());
		   quark_etas.push_back(lightJet->eta());
		   quark_phis.push_back(lightJet->phi());
		   quark_ids.push_back(lightJet->pdgId());
		   quark_ordered.push_back(std::make_pair(nQuark, lightJet->pt()));
                   nQuark++;
...
/*
void
GenAnalyzer_st::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void
GenAnalyzer_st::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void
GenAnalyzer_st::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void
GenAnalyzer_st::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
DEFINE_FWK_MODULE(GenAnalyzer_st);
