...
// user include files
#include "GeneratorInterface/GenFilters/interface/GenAnalyzer.h"
...
const reco::Candidate* GenAnalyzer::getGeneratorFinalStateFor(const reco::Candidate *p, bool isSherpa) {
...
bool GenAnalyzer::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	for (size_t i = 0; i < genParticles->size(); ++i ) {
		...
		if(abs(p.pdgId()) ==  24 && (p.status()==3 || p.status()==22)) {

			const reco::Candidate *w = getGeneratorFinalStateFor(&p); // Find the w

			WPt  = w->pt();
                        WEta  = w->eta();

			int nWDau = w->numberOfDaughters();

			for (int j = 0; j < nWDau; ++j) { // Loop on the w daughters

				const reco::Candidate *w_daughter;

				if (w->daughter(j)->status() == 3) 
                                  w_daughter = w->daughter(j); // pythia 6

				else
                                  w_daughter = getGeneratorFinalStateFor(w->daughter(j));
